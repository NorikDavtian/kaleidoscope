        ...
        let params = {
            ...
            orbTint: 'white', // white | ink | palette | warm | cool | depth | custom
            orbColor: '#6fb4ff',  // the custom tint's colour
        ...
        const BASE_PLATES = [
            ...
            { id: 'interference', name: 'Interference', anim: true }
        ];
        ...
        // Playback settings for whatever video is on: the piece of it that
        // loops, and whether it is heard. A live camera has neither — it has
        // no duration to trim and nothing to hear.
        let vidTrimA = 0, vidTrimB = 0, vidMuted = false, vidTrimmable = false;

        function syncVideoControls() {
            const box = document.getElementById('video-controls');
            if (box) box.classList.toggle('off', !vidTrimmable);
            const m = document.getElementById('video-mute-toggle');
            if (m) {
                m.className = vidMuted ? 'switch' : 'switch on';
                m.textContent = vidMuted ? 'Off' : 'On';
                m.setAttribute('aria-pressed', !vidMuted);
            }
            if (!vidTrimmable) return;
            const dur = (plateVideo && isFinite(plateVideo.duration)) ? plateVideo.duration : 0;
            [['videoTrimA', vidTrimA], ['videoTrimB', vidTrimB]].forEach(function (p) {
                const s = document.getElementById(p[0]);
                if (s) { s.max = dur.toFixed(2); s.value = p[1]; }
                const d = document.getElementById(p[0] + '-value');
                if (d) d.textContent = p[1].toFixed(1) + 's';
            });
        }

        // Called when a video becomes the source: the trim opens to the whole
        // clip, and the sound follows whose video it is.
        function adoptVideoPlayback(muted) {
            vidTrimmable = !!(plateVideo && isFinite(plateVideo.duration) && plateVideo.duration > 0);
            vidTrimA = 0;
            vidTrimB = vidTrimmable ? plateVideo.duration : 0;
            vidMuted = !!muted;
            if (plateVideo) plateVideo.muted = vidMuted;
            syncVideoControls();
        }

        function setVideoTrim(which, v) {
            const t = parseFloat(v) || 0;
            const dur = (plateVideo && isFinite(plateVideo.duration)) ? plateVideo.duration : 0;
            // A quarter second of clearance, or the loop would land on its own
            // end point and stutter in place.
            if (which === 'a') vidTrimA = Math.min(t, Math.max(0, vidTrimB - 0.25));
            else vidTrimB = Math.max(t, Math.min(dur, vidTrimA + 0.25));
            if (plateVideo && plateVideo.currentTime < vidTrimA) plateVideo.currentTime = vidTrimA;
            syncVideoControls();
        }
        ...
        function toggleVideoMute() {
            vidMuted = !vidMuted;
            if (plateVideo) {
                plateVideo.muted = vidMuted;
                // Unmuting a clip the browser silently muted to let it autoplay
                // needs the play call repeated, or it stays quiet.
                if (!vidMuted && plateVideo.paused) plateVideo.play().catch(function () {});
            }
            syncVideoControls();
        }

        // Per frame while a video is live: keep playback inside the trim.
        function holdVideoTrim() {
            if (!vidTrimmable || !plateVideo || vidTrimB <= vidTrimA) return;
            const t = plateVideo.currentTime;
            if (t >= vidTrimB || t < vidTrimA - 0.25) plateVideo.currentTime = vidTrimA;
        }
        ...
        function useCamera() {
            ...
            navigator.mediaDevices.getUserMedia({
                ...
                plateVideo.onloadeddata = function () {
                    ...
                    setSource('image');
                    adoptVideoPlayback(true);   // a live feed has nothing to trim
        ...
        function ensureVideoEl() {
            ...
            // The trim is held on the element's own clock as well as in the
            // frame loop: a backgrounded tab throttles rAF to nothing while
            // the video keeps playing, and the clip would run past its end.
            plateVideo.addEventListener('timeupdate', holdVideoTrim);
        ...
        function useVideoPlate(meta, name) {
            ...
            plateVideo.onloadeddata = function () {
                ...
                setSource('image');
                adoptVideoPlayback(true);      // built-ins open silent
                plateVideo.play().catch(function () {});
                loop();
            };
        ...
        function qcInit() {
            ...
            qcCv = document.createElement('canvas');
            ...
            for (let i = 0; i < QC_WAVES; i++) {
                ...
                for (let c = 0; c < 3; c++) {
                    ...
                    for (let y = 0; y < QC_SIDE; y++) {
                        const vy = (y / QC_SIDE - 0.5) * QC_SPAN;
                        for (let x = 0; x < QC_SIDE; x++, p++) {
                            const vx = (x / QC_SIDE - 0.5) * QC_SPAN;
                            // The small per-channel offset decorrelates the
                            // channels near the centre, where every wave
                            // would otherwise peak together and burn white.
                            const t = QC_KS[c] * (dx * vx + dy * vy) + c * 0.6;
                            C[p] = Math.cos(t);
                            S[p] = Math.sin(t);
                        }
                    }
        ...
        function qcPaint(tau) {
            ...
            for (let c = 0; c < 3; c++) {
                ...
                const out = qcInt[c];
                for (let p = 0; p < Npx; p++) {
                    const v = (qcRe[p] * qcRe[p] + qcIm[p] * qcIm[p]) * norm;
                    out[p] = Math.pow(v, 0.7) * 1.5;
                }
            }
        ...
        // Only the welcome disc runs the particle shell now — the breathing
        // guide draws itself as discrete balls, in drawOrb below.
        ...
        // The tint, resolved to a colour pair the ball lerps across.
        function orbBallColors() {
            const p = params.colorPalette;
            switch (params.orbTint) {
                case 'palette': return [hexToRgb(p[1]), hexToRgb(p[4])];
                case 'warm':    return [[255, 217, 160], [224, 90, 58]];
                case 'cool':    return [[183, 227, 255], [90, 109, 255]];
                case 'depth':   return [hexToRgb(p[4]), hexToRgb(p[1])];
                case 'ink':     return [[24, 24, 30], [72, 72, 82]];
                case 'custom': {
                    // One chosen colour, darkened along the lerp so the far
                    // side of the shell still reads as depth rather than flat.
                    const h = hexToRgb(params.orbColor);
                    return [h, [h[0] * 0.42, h[1] * 0.42, h[2] * 0.42]];
                }
                default:        return [[246, 245, 240], [168, 168, 176]];
            }
        }

        let breathBallSpin = 0;

        // The guide in the equalizer ball's language: discrete glowing balls
        // rather than particle fog, laid on a Fibonacci sphere so it reads as
        // a clean orb, turning slowly, with a soft radial ripple the breath
        // deepens. The whole thing still swells on the ring's CSS transform.
        function drawOrb() {
            if (params.guideStyle !== 'orb') return;   // the circle owns the guide
            const cv = document.getElementById('breath-ring');
            if (!cv) return;
            const side = Math.max(80, Math.min(620, Math.round(params.orbSize)));
            ...
            const R = c * 0.74;
            const spin = breathBallSpin;
            const ripple = 0.025 + breathValue * 0.05;
            const strength = Math.min(2, Math.max(0, params.orbStrength));
            const pair = orbBallColors();
            const a = pair[0], b = pair[1];

            for (let i = 0; i < N; i++) {
                const phi = Math.acos(1 - 2 * (i + 0.5) / N);
                const th = i * GA + spin;
                const r = R * (1 + Math.sin(spin * 2 + i) * ripple);
                const sp = Math.sin(phi);
                const x = r * sp * Math.cos(th);
                const y = r * Math.cos(phi);
                const z = r * sp * Math.sin(th);

                const depth = 0.35 + 0.65 * (z / R + 1) / 2;
                const k = i / N;
                const size = (1.1 + 2.1 * depth) * (side / 300);

                g.fillStyle = 'rgba(' +
                    Math.round(a[0] + (b[0] - a[0]) * k) + ',' +
                    Math.round(a[1] + (b[1] - a[1]) * k) + ',' +
                    Math.round(a[2] + (b[2] - a[2]) * k) + ',' +
                    Math.min(1, 0.85 * depth * strength).toFixed(3) + ')';
                g.beginPath();
                g.arc(c + x, c + y, size, 0, 6.28318);
                g.fill();
            }

            breathBallSpin += 0.006;
        }
        ...
        function circleColors() {
            ...
            switch (params.orbTint) {
                ...
                case 'custom': return params.orbColor;
        ...
        function setOrbColor(hex) {
            params.orbColor = hex;
            setOrbTint('custom');     // picking a colour is choosing it
        }
        ...
        let discoWaveTint = 'palette';   // palette | warm | cool | white | custom
        let discoWaveColor = '#6fb4ff';
        let discoBg = 'high';      // low | medium | high — the artwork's floor brightness

        // At Low the artwork rests faint and flares with the signal; High
        // never dims. The floor is what the silence looks like.
        const DISCO_BG_BASE = { low: 0.22, medium: 0.5, high: 1 };

        function setDiscoWaveTint(t) {
            discoWaveTint = t;
            ['palette', 'warm', 'cool', 'white'].forEach(function (k) {
                const el = document.getElementById('wave-' + k);
                if (el) el.className = (k === discoWaveTint) ? 'active' : '';
            });
        }

        function setDiscoWaveColor(hex) {
            discoWaveColor = hex;
            setDiscoWaveTint('custom');
        }

        function discoWaveRGB() {
            switch (discoWaveTint) {
                case 'warm':   return [255, 158, 92];
                case 'cool':   return [111, 180, 255];
                case 'white':  return [246, 245, 240];
                case 'custom': return hexToRgb(discoWaveColor);
                default: {
                    // The adaptive one: a slow lerp between two palette notes.
                    const p2 = hexToRgb(params.colorPalette[2]);
                    const p4 = hexToRgb(params.colorPalette[4]);
                    const k = 0.5 + 0.5 * Math.sin(waveA * 0.05);
                    return [Math.round(p2[0] + (p4[0] - p2[0]) * k),
                            Math.round(p2[1] + (p4[1] - p2[1]) * k),
                            Math.round(p2[2] + (p4[2] - p2[2]) * k)];
                }
            }
        }

        function setCanvasDim(v) {
            const w = document.getElementById('canvas-wrap');
            if (w) w.style.opacity = v >= 1 ? '' : v.toFixed(3);
        }

        function setDiscoBg(level) {
            discoBg = DISCO_BG_BASE[level] ? level : 'high';
            ['low', 'medium', 'high'].forEach(function (k) {
                const el = document.getElementById('disco-bg-' + k);
                if (el) el.className = (k === discoBg) ? 'active' : '';
            });
            if (!disco || discoBg === 'high') setCanvasDim(1);
        }
        ...
        function startDiscoAudio() {
            ...
            // The browser's voice-call processing is poison for a music
            // visualizer: noise suppression decides steady music is background
            // noise and ducks it to silence within seconds, and auto-gain
            // squashes the rest. Ask for the raw feed.
            const wants = {
                echoCancellation: false,
                noiseSuppression: false,
                autoGainControl: false
            };
            if (discoMicId) wants.deviceId = { exact: discoMicId };
            navigator.mediaDevices.getUserMedia({ audio: wants }).then(function (stream) {
                ...
                // Held in a durable reference: an unreachable stream-source
                // node is fair game for the collector, and its death reads as
                // the microphone going quiet mid-set.
                discoSrcNode = discoAudioCtx.createMediaStreamSource(stream);
                discoAnalyser = discoAudioCtx.createAnalyser();
                discoAnalyser.fftSize = 512;
                discoAnalyser.smoothingTimeConstant = 0.55;
                discoSrcNode.connect(discoAnalyser);
        ...
        function discoStep(f60) {
            ...
            // The background dimmer: silence rests at the floor, the signal
            // lifts it toward full light. Opacity only — compositor-cheap.
            const bgBase = DISCO_BG_BASE[discoBg];
            if (bgBase < 1) setCanvasDim(Math.min(1, bgBase + (1 - bgBase) * discoLevel * 1.6));
        ...
        function drawDiscoWaves(g, side) {
            ...
            const rgb = discoWaveRGB();
            g.strokeStyle = 'rgba(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ',0.5)';
