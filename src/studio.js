        ...
        function buildOrbPoints() {
            ...
            for (let i = 0; i < ORB_POINTS; i++) {
                ...
                const coarse = fbm3(x * 2.1, y * 2.1, z * 2.1, 3);
                const fine   = fbm3(x * 6.4, y * 6.4, z * 6.4, 2);
                const d = coarse + fine * 0.35;

                if (d < -0.06) continue;                    // a void
                const b = Math.min(1, (d + 0.06) * 1.9);
                if (b < 0.02) continue;

                // A thin shell. Letting the radius wander as far as it did
                // filled the volume, which is why it stopped reading as hollow.
                const rad = 0.95 + fine * 0.05 + (rnd() - 0.5) * 0.02;
                pts[n * 4]     = x * rad;
                pts[n * 4 + 1] = y * rad;
                pts[n * 4 + 2] = z * rad;
                pts[n * 4 + 3] = b;
        ...
        function renderOrb(el, state, gain, spinStep, tilt) {
            ...
            for (let i = 0; i < N; i++) {
                ...
                const depth = 0.30 + 0.70 * ((z2 + 1) * 0.5);
                let v = orbPts[o + 3] * depth * gain * 330;
                if (v <= 0) continue;
