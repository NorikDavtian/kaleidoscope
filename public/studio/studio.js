        ...
        // its own dial, never with the phases.
        function qcEnvelope() {
            if (qcEnvArr && qcEnvVal === qcParams.env) return qcEnvArr;
            const Npx = QC_SIDE * QC_SIDE;
            if (!qcEnvArr) qcEnvArr = new Float32Array(Npx);
            let p = 0;
            for (let y = 0; y < QC_SIDE; y++) {
                const ey = (y / QC_SIDE - 0.5) * 2;
                for (let x = 0; x < QC_SIDE; x++, p++) {
                    const ex = (x / QC_SIDE - 0.5) * 2;
                    qcEnvArr[p] = Math.exp(-qcParams.env * 1.9 * (ex * ex + ey * ey));
                }
            }
            qcEnvVal = qcParams.env;
            return qcEnvArr;
        }
